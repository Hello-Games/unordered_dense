...
TEST_CASE_MAP("change_key", std::string, regular_type) {
    map_t map;
    ...
    auto find_a = map.find("a");
    auto change_a = map.change_key(find_a, "x");
    REQUIRE(change_a.second);
    REQUIRE(change_a.first == find_a);
    REQUIRE(change_a.first == map.find("x"));
    REQUIRE(change_a.first->first == "x");
    REQUIRE(change_a.first->second == regular_type(1U, "b"));
    REQUIRE(map.find("a") == map.end());

    auto find_c = map.find("c");
    auto change_c = map.change_key(find_c, "x");
    REQUIRE_FALSE(change_c.second);
    REQUIRE(change_c.first == map.find("x"));
    REQUIRE(change_a.first->first == "x");
    REQUIRE(change_c.first->second == regular_type(1U, "b"));
    REQUIRE(map.find("c")->first == "c");
    REQUIRE(map.find("c")->second == regular_type(2U, "d"));
}
